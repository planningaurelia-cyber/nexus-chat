...
export default function EvilEye({
  ...
  useEffect(() => {
    ...
    let nextWanderChangeAt = 0;

    function pickNewWanderTarget(now) {
      const isDart = Math.random() < 0.3;
      const isPause = Math.random() < 0.2;

      if (isPause) {
        nextWanderChangeAt = now + 1200 + Math.random() * 1000;
        return;
      }

      const range = isDart ? 0.85 : 0.4;
      wanderTarget = {
        x: (Math.random() * 2 - 1) * range,
        y: (Math.random() * 2 - 1) * range
      };
      nextWanderChangeAt = now + (isDart ? 500 : 900) + Math.random() * 1200;
    }

    function setRealTarget(nx, ny) {
      mouse.tx = nx;
      mouse.ty = ny;
    ...
    // Pause rendering when the tab isn't visible, to save battery
    ...
    function update(time) {
      ...
      const recentRealInput = now - lastInputTime < INPUT_HOLD_MS;
      ...
      if (recentRealInput) {
        ...
        if (now >= nextWanderChangeAt) pickNewWanderTarget(now);
        targetX = wanderTarget.x;
        targetY = wanderTarget.y;
      }

      const ease = recentRealInput ? 0.08 : 0.035;
      mouse.x += (targetX - mouse.x) * ease;
      mouse.y += (targetY - mouse.y) * ease;
